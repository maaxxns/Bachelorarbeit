...
Through measurement of the change in polarization, it is then possible to determine the intensity of the electric field of the $\si{\tera\hertz}$-pump beam.
For this two photodiodes are used, which measure the probe beam intensity in dependence on its polarization.
It is explained in section \ref{sec:setup} how the beam is split intp its two polarization-dependent parts.
...
it is possible to determine the field strength $E_\text{THz}$ \cite[7]{THZ_eltric_field}.
With the electricfield it is then possible to calculate the peak power of the $\si{\tera\hertz}$-electric field.
